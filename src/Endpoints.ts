export namespace Endpoint {
  ...
  export namespace Incidents {
    export function all(): string {
      return `/${API_BASE}/${INCIDENTS_JSON}`;
    }

    export function unresolved(): string {
      return `/${API_BASE}/${INCIDENTS}/${UNRESOLVED_JSON}`;
    }
  }

  export namespace ScheduledMaintenances {
    export function active(): string {
      return `/${API_BASE}/${SCHEDULED_MAINTENANCES}/${ACTIVE_JSON}`;
    }
    export function all(): string {
      return `/${API_BASE}/${SCHEDULED_MAINTENANCES_JSON}`;
    }
    export function upcoming(): string {
      return `/${API_BASE}/${SCHEDULED_MAINTENANCES}/${UPCOMING_JSON}`;
    }
  }

  export function components(): string {
    return `/${API_BASE}/${COMPONENTS_JSON}`;
  }

  export function summary(): string {
    return `/${API_BASE}/${SUMMARY_JSON}`;
  }

  export function status(): string {
    return `/${API_BASE}/${STATUS_JSON}`;
  }

  export function subscribers(): string {
    return `/${API_BASE}/${SUBSCRIBERS_JSON}`;
  }
}
